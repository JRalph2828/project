    ...
    { case StmtK:
        ...
        { case CompK:
            ...
            case RetK:
            {
              ...
              typeError(t,"return value should not exist");
            } else if (funcType == Integer &&
                (expr == NULL || expr->type == Void)) {
              typeError(t,"return value sholud exist");
            } else if (expr->type == IntegerArray) {
                typeError(t,"wrong return value type");
            }
          }
          break;
        default:
      ...
      { case AssignK:
        ...
        case ArrIdK:
          { const char *symbolName = t->attr.name;
            ...
            TreeNode *symbolDec = NULL;

            if (bucket == NULL)
              break;
            symbolDec = bucket->treeNode;

            if (t->kind.exp == ArrIdK) {
              if (symbolDec->kind.dec != ArrVarK &&
                  symbolDec->kind.param != ArrParamK)
                typeError(t,"expected array symbol");
              else
                t->type = Integer;
            } else {
              t->type = symbolDec->type;
            }
          }
          break;
        case CallK:
          { const char *callingFuncName = t->attr.name;
            BucketList bl = st_bucket(callingFuncName);
            const TreeNode * funcDec = NULL;
            if (bl) {
              funcDec = bl->treeNode;
            }
            TreeNode *arg;
            ...
            arg = t->child[0];
            param = funcDec->child[1];

            if (funcDec->kind.dec != FuncK)
