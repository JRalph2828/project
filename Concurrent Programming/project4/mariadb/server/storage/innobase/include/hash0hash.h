...
#define HASH_INSERT2(TYPE, NAME, TABLE, FOLD, DATA)\
do {\
    hash_cell_t*    cell3333;\
    TYPE*       struct3333;\
\
...
\
    if (cell3333->node == NULL) {\
        while(!(__sync_bool_compare_and_swap(&cell3333->node, NULL, DATA))){};\
    } else {\
        struct3333 = (TYPE*) cell3333->node;\
\
        while (1){\
            struct3333 = (TYPE*) struct3333->NAME;\
\
            if(!(__sync_bool_compare_and_swap(&struct3333->NAME, NULL, DATA))){\
                continue;\
            }\
\
            break;\
