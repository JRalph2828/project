...
RESULT_FMTS = "Result: %d/%d (%.2f) %.2f secs %.2f (find) secs"
...
def test_case(arr):
    ...
    return succ, end - start, end_find - start_find

def test_case_seq(casename, case_size):
    print(casename + " Test")
    
    result, elapse, elapse2 = test_case(range(case_size))
    
    print(RESULT_FMTS % (result, case_size, float(result)/case_size * 100, elapse, elapse2))
    if (result != case_size):
...
def test_case_rnd(casename, case_size):
    ...
    result, elapse, elapse2 = test_case(arr)
...
def test_delete_random(casename, pick):
    ...
    result, elapse, elapse2 = test_delete(arr[pick:], arr[:pick])
    print(RESULT_FMTS % (result, LARGE_CASE, float(result)/(LARGE_CASE) * 100, elapse, elapse2))
...
def test_delete_seq():
    ...
    result, elapse, elapse2 = test_delete([], arr)
    print(RESULT_FMTS % (result, LARGE_CASE, float(result)/(LARGE_CASE) * 100, elapse, elapse2))    
    if (result != LARGE_CASE):
...
def test_delete_chunk():
    ...
    result, elapse, elapse2 = test_delete(arr, deleted)
    print(RESULT_FMTS % (result, LARGE_CASE, float(result)/(LARGE_CASE) * 100, elapse, elapse2))
    if (result != LARGE_CASE):
        print("FAILED - Stop Testing")
        exit();
